                            ...
                            return key === "name" ? null : (
                                <>
                                    <tr>
                                        ...
                                        <td colSpan={2} width={400 * 1} style={{ fontWeight: key === "type" ? "bold" : "normal" }} className={`baseline align-middle ${key === "composition" ? `text-white bg-black` : ``}`}>{key === "type" ? `EMA - ${selectedVaccine.name}` : getProductProfileValueByVaccineNameAndType("EMA", key, selectedVaccine.name)}</td>
                                        <td colSpan={2} width={400 * 1} style={{ fontWeight: key === "type" ? "bold" : "normal" }} className={`baseline align-middle ${key === "composition" ? `text-white bg-black` : ``}`}>{key === "type" ? `FDA - ${selectedVaccine.name}` : getProductProfileValueByVaccineNameAndType("FDA", key, selectedVaccine.name)}</td>
                                        <td colSpan={2} width={400 * 1} style={{ fontWeight: key === "type" ? "bold" : "normal" }} className={`baseline align-middle ${key === "composition" ? `text-white bg-black` : ``}`}>{key === "type" ? `WHO - ${selectedVaccine.name}` : getProductProfileValueByVaccineNameAndType("WHO", key, selectedVaccine.name)}</td>
                                    </tr>
                                </>
                            )
                        })}
